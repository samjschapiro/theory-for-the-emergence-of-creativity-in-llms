...
def preprocess_data_bart(data):
    if len(data) == 4:
        ...
        input_text, target_text, target_labels, lm_tokenizer, args = data
    ...
    if target_labels is None:
        ...
        target_labels = target_labels["input_ids"].squeeze(0)
        lm_labels = target_labels.clone()
        assert lm_labels.shape == target_ids.shape, f"{lm_labels.shape} != {target_ids.shape}"
...
class SimpleSummarizationDataset(Dataset):
    def __init__(self, lm_tokenizer, args, data, mode):
        ...
        if os.path.exists(cached_features_file) and (
            ...
            if "target_labels" in data.columns:
                print("here")
                data = [
                    (input_text, target_text, target_labels, lm_tokenizer, args)
                    for input_text, target_text, target_labels in zip(data["input_text"], data["target_text"], data["target_labels"])
                ]
            else:
                data = [
