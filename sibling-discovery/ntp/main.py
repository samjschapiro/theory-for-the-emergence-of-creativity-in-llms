...
def main():
    ...
    parser.add_argument("--save_step_dense_interval", default=5000, type=int, help="")
    ...
    parser.add_argument('--temperature', default=None, type=float, help='Temperature for sampling')
    ...
    if args.do_predict or args.predict_during_training:
        if args.custom_test:
            test_df = read_data_source_target(os.path.join(args.data_dir, args.custom_test), return_json=True, cutoff=args.prediction_cutoff)
        else:
            test_df = read_data_source_target(os.path.join(args.data_dir, "test.json"), return_json=True, cutoff=args.prediction_cutoff)
    else:
        test_df = None
