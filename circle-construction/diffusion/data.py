...
def get_dataset(name, mode, tokenizer, cache_dir=None, block_size=1024, num_proc=8):
    if name == "wikitext103":
    ...
    elif "in_context" in name:
        # data is a dataframe with one column "text"
        with open(name, "r") as f:
            data = json.load(f)
        target_texts = [item["target_text"] for item in data]
        dataset = Dataset.from_dict({"text": target_texts})
    else:
    ...
    def group_texts(examples):
        if block_size == 29:
            ...
            # Just pad each example to block_size
            return {k: [t + [EOS] * (block_size - len(t)) for t in examples[k]] for k in examples.keys()}
...
def get_dataloaders(config, tokenizer, distributed=True):
    ...
    train_set = get_dataset(config.data.train, "train", tokenizer, cache_dir=config.data.cache_dir, block_size=config.model.length)
    valid_set = get_dataset(config.data.valid, "validation" if config.data.valid != "text8" else "test", tokenizer, cache_dir=config.data.cache_dir, block_size=config.model.length)
