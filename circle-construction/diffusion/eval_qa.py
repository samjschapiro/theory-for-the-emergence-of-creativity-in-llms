...
class Evaluator:
    ...
    def eval_file(self, fn='all_items.json'):
        ...
        temp.sort(key=lambda var: int(var[0].split("_")[1].split(".")[0]))
    ...
    def get_validity_score(self, pred):
        ...
        # Make sure that the nodes form a circle
        if not edges:
            return 0
        ...
        # Each node should appear exactly twice in a circle
        if not all(count == 2 for count in node_counts.values()):
            return 0
            
        # Check if edges form a single connected cycle
        ...
        # Start from any node and traverse
        visited = set()
        ...
        while curr not in visited:
            ...
            if next_node is None:
                # Check if we can reach back to start
                if start_node in adj[curr] and len(visited) == len(node_counts):
                    return 1
                return 0
            prev = curr
            curr = next_node
...
def main():
    ...
    for folder_name, scores in scores_dict:
        step = int(folder_name.split('_')[1].split('.')[0])
    ...
    df.to_csv(args.dir.split("/train.json")[0].split("/")[-1] + ".csv", index=False)
    ...
    plt.savefig(args.dir.split("/train.json")[0].split("/")[-1] + ".png")
