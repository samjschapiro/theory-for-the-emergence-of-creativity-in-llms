...
def _run(rank, world_size, cfg):
    ...
    # If add_vocab is true, then we need to add the vocab to the model
    if cfg.add_vocab:
        with open(cfg.add_vocab, 'r') as file:
            added_tokens = json.load(file)
        cfg.tokens = cfg.tokens + len(added_tokens)
    ...
    pretrained_score_model = SEDD.from_pretrained("louaaron/sedd-{}".format(cfg.model.name))
    # Load the pretrained score model's parameters into the score model
    state_dict = pretrained_score_model.state_dict()
    skip = [
        "vocab_embed.embedding", "output_layer.linear.weight", "output_layer.linear.bias"
    ]
    state_dict = {k: v for k, v in state_dict.items() if k not in skip}
    score_model.load_state_dict(state_dict, strict=False)
    print("Loaded pretrained score model parameters into score model")
